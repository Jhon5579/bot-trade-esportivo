...
def rodar_construtor():
    """Fase 1: Atualiza o catálogo com novos times da The Odds API."""
    ...
    if not lista_mestra:
        print(f"❌ ERRO: Arquivo '{ARQUIVO_LISTA_MESTRA}' essencial não encontrado."); return False

    print(f"Catálogo atual com {len(catalogo)} times. Fonte de dados com {len(lista_mestra)} times.")

    # Busca times na The Odds API
    ...
    try:
        ...
        print(f"  > ERRO de conexão ao buscar jogos: {e}"); return False
    ...
    if not times_novos:
        print("✅ Todos os times de hoje já estão no catálogo."); return True
...
def rodar_mapeador():
    """Fase 2: Atualiza o mapa de nomes (CSV -> Catálogo)."""
    print("\n--- 🗺️ FASE 2: EXECUTANDO MAPEADOR DE NOMES... 🗺️ ---")

    mapa_de_nomes = carregar_json(ARQUIVO_MAPA_SAIDA)
    catalogo = carregar_json(ARQUIVO_CATALOGO)
    if not catalogo:
        print("❌ ERRO: Catálogo de times está vazio. Rode o construtor primeiro."); return False

    try:
        df = pd.read_csv(ARQUIVO_CSV_HISTORICO, encoding='latin1')
        ...
        print(f"❌ ERRO: Arquivo '{ARQUIVO_CSV_HISTORICO}' não foi encontrado."); return False

    nomes_api_referencia = set(catalogo.keys())

    if not nomes_csv_a_mapear:
        print("✅ Todos os times do CSV já foram mapeados."); return True
    ...
    for nome_csv in sorted(nomes_csv_a_mapear):
        melhor_partida_api, maior_pontuacao = max(
            ((nome_api, fuzz.ratio(nome_csv.lower(), nome_api.lower())) for nome_api in nomes_api_referencia),
            key=lambda item: item[1],
            default=(None, 0)
        )
...
def rodar_corretor():
    """Fase 3: Aplica o mapa para corrigir o arquivo CSV."""
    print("\n--- ⚙️ FASE 3: EXECUTANDO CORRETOR DE CSV... ⚙️ ---")

    mapa_de_nomes = carregar_json(ARQUIVO_MAPA_SAIDA)
    if not mapa_de_nomes:
        print("❌ ERRO: O arquivo de mapa está vazio. Rode o mapeador primeiro."); return False

    try:
        df = pd.read_csv(ARQUIVO_CSV_HISTORICO, encoding='latin1')
    except FileNotFoundError:
        print(f"❌ ERRO: O arquivo '{ARQUIVO_CSV_HISTORICO}' não foi encontrado."); return False

    print("Aplicando regras de correção ao banco de dados...")
